...
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix, const bool is_fisheye)
{
	...
	if (is_fisheye) // panorama
	{
		...
		J = glm::mat3(
			...
			-focal_y * t.x * t.y * len_inv * x2y2z2_inv * inv_pi, focal_y * len * x2y2z2_inv * inv_pi, -focal_y * t.y * t.z * x2y2z2_inv * len_inv * inv_pi,
			0.0f, 0.0f, 0.0f
		);
		// J = glm::mat3(
		// 	focal_x * t.z * x2z2_inv * inv_2pi, 0.0f, -focal_x * t.x * x2z2_inv * inv_2pi,
...
__global__ void preprocessCUDA(int P, int D, int M,
	...
	const float* projmatrix, //world2ndc, not cam2ndc
	...
	bool prefiltered)
{
	...
	if (is_fisheye) // panorama
	{
		...
		float p_w = 1.0f / (sqrt(p_hom.x * p_hom.x + p_hom.y * p_hom.y + p_hom.z * p_hom.z) + EPS);
		float3 p1 = { p_hom.x * p_w, p_hom.y * p_w, p_hom.z * p_w};
		const float pi = 3.1415926536f;
		p_proj.x = atan2(p1.x, p1.z) / pi;
		p_proj.y = atan2(p1.y, sqrt(p1.x * p1.x + p1.z * p1.z)) * 2.0 / pi;
		// p_proj.y = 2.0f * atan2(p_proj.y, p_proj.z) / pi;
		p_proj.z = 0;
	...
	float* __restrict__ out_color)
{
...
#if SEAM == 1
	__shared__ int collected_radius[BLOCK_SIZE];
#endif
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		#if SEAM == 1
			collected_radius[block.thread_rank()] = radii[coll_id];
		#endif
		}
		block.sync();
		...
		#if SEAM == 1 
			xy.x = getGx(collected_radius[j], xy.x, pixf.x, grid); //Ghost Gaussians
		#endif
